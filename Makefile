tag=duckietown_aido_ros_bridge

regular_packages=duckietown_aido_ros_bridge
test_packages=duckietown_aido_ros_bridge_tests
...
run-with-mounted-src:
	mkdir -p out-docker
	docker run -it -v $(PWD)/src:/duckietown_aido_ros_bridge/src:ro -v $(PWD)/out-docker:/out $(tag) dt-pc-demo
...
upload: # v3
	...
	aido-check-tagged
	aido-check-need-upload --package duckietown-aido-ros-bridge-daffy make upload-do
